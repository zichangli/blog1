...
`location.pathname`可获取当前的访问路径，利用该方式可设置菜单选项的选中状态，这里需注意，在查看文章时，只要`location.pathname`以`/blog`开头，文章就被认为是选中状态，所以需要在data中的文章数据部分添加startWith属性，默认值为true
...
mockjs的基本使用`Mock.mock(url, type, template)`，第一个参数为设置的拦截规则地址，当请求地址以
...
页面标题可通过更改document.title的值来实现切换
...
3. 虚拟dom是如何转换为真实dom的？
   ...
   **vue最终还是会生成真实DOM节点，并挂载到页面上，因此，如果页面后续没有改动，虚拟dom的方式效率要低于直接渲染真实dom**
...
<img src="D:\Desktop\前端资料\笔记\image\20210226153448.png" alt="image-20210225144108143" style="zoom:50%;" align="center" />

Observer是vue内部的**构造器**，我们可以通过Vue提供的静态方法`Vue.observable( object )`间接的使用该功能。
...
<img src="D:\Desktop\前端资料\笔记\image\20210226154624.png" alt="image-20210225144108143" style="zoom:50%;" align="center" />
...
<img src="D:\Desktop\前端资料\笔记\image\20210226155852.png" alt="image-20210225144108143" style="zoom:50%;" align="center" />
...
<img src="D:\Desktop\前端资料\笔记\image\20210226161404.png" alt="image-20210225144108143" style="zoom:50%;" align="center" />
...
<img src="D:\Desktop\前端资料\笔记\image\20210226163936.png" alt="image-20210225144108143" style="zoom:50%;" align="center" />
...
3. `patch`函数的对比流程
   ...
   **术语解释**：
   ...
   1. 「**相同**」：是指两个虚拟节点的标签类型、`key`值均相同，但`input`元素还要看`type`属性，key值默认为undefined
...
1. 创建vue实例和创建组件的流程基本一致
   ...
   3. 进入注入流程：处理属性、computed、methods、data、provide、inject，最后使用代理模式将它们挂载到实例中（**将数据转换成响应式数据，使用bind将所有函数绑定到vue实例中，这也是vue实例中所有methods指向this的原因**）
...
区别：transition只控制动画，不会生成元素，而transition-group会生成元素，默认生成一个span标签，可通过tag来控制生成元素的类型

`Transision`可以监控其内部的**单个dom元素**的出现和消失，并为其附加样式

如果要监控一个dom列表，就需要使用`TransitionGroup`组件

它会对列表的新增元素应用**进入效果**，删除元素应用**消失效果**，对被移动的元素应用`v-move`样式，当设置name后，用name-move进行设置
...
冻结的对象不会被响应化，用于一些不会变动的数据，通过Object.freeze()实现

### 使用函数式组件

参见[函数式组件](https://cn.vuejs.org/v2/guide/render-function.html#%E5%87%BD%E6%95%B0%E5%BC%8F%E7%BB%84%E4%BB%B6)

函数式组件意味着该组件没有状态（没有响应式数据），也没有实例，即没有正常组件的声明周期函数等，因此在脚本执行过程中会提升效率
...
问题描述：当一个页面为一个很长的列表，且列表每一项中都包含很多元素时，用户浏览过程中肯能会在一页中加载过多的元素从而引发页面卡顿，占用过多内存

解决思路：将列表容器设置为固定高度，其内部子元素设置为绝对定位，限制一定的数量，如当前页面显示数据1、2、3、4、5、6，在上拉过程中，当数据1超过视窗顶部时，将列表数据下移，并改动列表中的数据，此时，列表中的数据显示为2、3、4、5、6、7
...
在vue-cli打包过程中会读取env文件，并替换一些环境变量，如NODE_ENV、BASE_URL，也可以自定义一些变量如VUE_APP_xxx样式的变量，均可进行替换
...
- devServer
- publicPath，用来配置打包后静态资源路径写法
- outputDir，用来配置打包输出路径
- runtimeCompiler，配置运行时编译
- transpileDependencies，默认情况下babel-loader会忽略所有node_modules中的文件
- configureWebpack，如果该值是一个对象，则会通过webpack-merge合并到最终的配置中
...
该命令可通过可视化的方式进行创建和管理工程

### vue add

1、vue add @vue/router
