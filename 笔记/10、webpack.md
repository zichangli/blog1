...
![](D:\Desktop\前端资料\Lesson One课件\13、DuYi-Webpack-master\1. webpack核心功能\1-8. 编译过程\assets\2020-01-09-10-26-15.png)
...
![](D:\Desktop\前端资料\Lesson One课件\13、DuYi-Webpack-master\1. webpack核心功能\1-8. 编译过程\assets\2020-01-09-12-43-52.png)
...
![](D:/Desktop/前端资料/Lesson One课件/13、DuYi-Webpack-master/1. webpack核心功能/1-11. loader/assets/2020-01-13-10-39-24.png)
...
**loader在匹配过程中，所有的loader组成一个数组，在运行时从后往前进行匹配和格式化**
...
![](D:/Desktop/前端资料/Lesson One课件/13、DuYi-Webpack-master/4. js兼容性/4-1. babel的安装和使用/assets/2020-02-07-10-27-30.png)
...
![分包简单流程](D:/Desktop/前端资料/Lesson One课件/13、DuYi-Webpack-master/5. 性能优化/5-6. 自动分包/assets/2020-02-24-17-19-47.png)
  ...
  optimization:{
    splitChunks: {
      ...
      cacheGroups: {
        ...
        vendors: { 
          test: /[\\/]node_modules[\\/]/, // 当匹配到相应模块时，将这些模块进行单独打包（括号中的斜杠为适配windows和Linux）
...
顾名思义，在打包过程中，某些模块并没有立即参与使用，可对其进行控制，当页面发生某些事件时再对该模块进行引入，但这里不可以使用require，require会进行语法树分析并将代码加入到main.js中，如以下代码，打包过程中tree-shaking会失效
...
oBtn.onclick = async function (){
    ...
    const result = chunk([2, 3, 4, 5, 6, 7], 2);
    console.log(result);
}
```

若要在打包过程中启用tree-shaking，可先将要使用的模块中的某一部分通过ES6模块化基本导出的方式保存为静态变量，然后在使用时异步加载保存过的静态变量
...
![](D:/Desktop/前端资料/Lesson One课件/13、DuYi-Webpack-master/5. 性能优化/5-12. gzip/assets/2020-02-28-15-37-26.png)
...
使用`compression-webpack-plugin`插件对打包结果进行预压缩，可以移除服务器的压缩时间，详细压缩方式等参数见官网

![](D:/Desktop/前端资料/Lesson One课件/13、DuYi-Webpack-master/5. 性能优化/5-12. gzip/assets/2020-02-28-15-41-44.png)
...
```js
// const module = document.getElementsByClassName("txt")[0].value;
//
// if (Math.random() > 0.5){
...
webpack-merge可对配置进行合并，合并方式为重复部分用后面的覆盖前面的
...
设置npm包下载地址：```npm config set registry https://registry.npm.taobao.org```

本地安装：```npm install 包名```或```npm i 包名```

全局安装：```npm install --global 包名```或```npm i -g 包名```

全局安装路径查看：```npm config get prefix```

初始化工程：```npm init```

初始化工程（全设置默认）：```npm init --yes```或```npm init -y```

根据package.json安装所有依赖：```npm install```或```npm i```

根据package.json安装生产环境依赖包：```npm install production```

安装依赖到生产环境：```npm i 包名```或```npm i --save 包名```或```npm i -S 包名```

安装依赖到开发环境：```npm i --save-dev 包名```或```npm i -D 包名```

精确安装最新版本：```npm install --save-exact 包名```或```npm install -E 包名```
...
查询安装包：```npm list [-g] [--depth=依赖深度]```

查看需要更新的包：```npm outdated```

更新包：```npm update [-g] [包名]``

卸载包：```npm uninstall [-g] 包名```

查询生效的配置属性：```npm config ls [-l] [--json]```

获取某个配置项：```npm config get 配置项```

设置某个配置项：```npm config set 配置项```

移除某个配置项：```npm config delet 配置项```

发布包（需将镜像源更换为npm官网）：

​	移除镜像源：```npm config delete registry```
...
webpack安装：```npm i -D webpack webpack-cli```

package.json命令：

​	开发环境打包：```"dev":"webpack --mode=development"```

​	开发环境实时打包：```"dev":"webpack --mode=development --watch"```
